...
一键装（推荐）：
```bash
git clone <repo> opencut-v3 && cd opencut-v3
...
- `DOUBAO_API_KEY` -- 多模态视觉，素材分析必需（强烈必填，否则文案对不上画面）
...
A: 检查 `DOUBAO_API_KEY` 是否配置且模型为 `doubao-seed-2-0-lite-260428`。素材分析靠
   doubao 多模态视觉，纯文本 provider 会幻觉。

**Q: 视频没声音**
A: TTS 用 minimax async（`t2a_async_v2`）。确认 `MINIMAX_API_KEY` 有效，看
   `data/projects/<id>/state.json` 的 `tts` 阶段 `error` 字段。
